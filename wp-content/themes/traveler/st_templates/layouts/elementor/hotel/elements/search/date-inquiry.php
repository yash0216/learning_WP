...
$hotel_booking_period_end = $hotel_booking_period + 1;
$start = STInput::get('start', date(TravelHelper::getDateFormat() , strtotime("+ {$hotel_booking_period} day")));
$end = STInput::get('end', date(TravelHelper::getDateFormat(), strtotime("+ {$hotel_booking_period_end} day")));
$date = STInput::get('date', date('d/m/Y h:i a'). '-'. date('d/m/Y h:i a', strtotime('+1 day')));
$has_icon = (isset($has_icon))? $has_icon: false;
?>
<div class="form-group form-date-field date-enquire form-date-search  <?php if($has_icon) echo ' has-icon '; ?>" data-format="<?php echo TravelHelper::getDateFormatMoment() ?>">
    <?php
        if($has_icon){
