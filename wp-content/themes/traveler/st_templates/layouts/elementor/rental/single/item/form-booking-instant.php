<div class="form-book-wrapper st-border-radius">
	...
	<div class="form-head">
		...
		if ( isset( $number_day ) && $number_day > 1 ) {
			echo wp_kses( sprintf( __( 'from <span class="price">%1$s</span> per %2$s nights', 'traveler' ), TravelHelper::format_money( $price ), $number_day ), [ 'span' => [ 'class' => [] ] ] );
		} else {
	...
	<?php } else { ?>
		<div class="submit-group mb30">
			...
			<form id="form-booking-inpage" class="form single-room-form rental-booking-form" method="post">
				...
				?>
				<div class="form-group st-external form-date-field form-date-hotel-room clearfix
				<?php
				if ( $has_icon ) {
					echo ' has-icon ';}
				?>
