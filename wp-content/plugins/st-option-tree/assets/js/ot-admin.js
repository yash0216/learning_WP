...
; (function ($) {
  OT_UI = {
      ...
      if (this.processing === false) {
        ...
        $.ajax({
          ...
          complete: function (data) {
            ...
            $('.type-textarea .wp-editor-area').attr("data-wpeditor","wpeditor");
            const items_editor = [
              ...document.querySelectorAll(`textarea[data-wpeditor^="wpeditor"]`),
            ];
            //var items_editor = document.querySelectorAll(`textarea[id^="tours_program_desc"]`);
            if (typeof items_editor !== 'undefined'){
              console.log(items_editor);
              items_editor.map(item => {
    ...
    getTemplateWidgetId : function( id ){
