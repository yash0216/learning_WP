...
if (! function_exists('ot_type_text')) {
    ...
    function ot_type_text($args = array())
    {
        ...
        // Build text input.
        if ($is_number) {
            echo '<input type="number" step="0.01" name="' . esc_attr($field_name) . '" id="' . esc_attr($field_id) . '" value="' . esc_attr($field_value) . '" class="widefat option-tree-ui-input ' . esc_attr($field_class) . '" />';
        } else {
...
function ot_type_wpeditor($args = array())
{
        // Turns arguments array into variables.
		extract( $args ); // phpcs:ignore
