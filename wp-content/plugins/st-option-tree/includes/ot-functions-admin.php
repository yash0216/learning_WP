...
if (! function_exists('ot_validate_setting')) {
    ...
    function ot_validate_setting($input, $type, $field_id, $wmpl_id = '')
    {
        ...
        } elseif (in_array($type, array( 'css', 'javascript', 'text', 'textarea', 'textarea-simple' , 'wpeditor' ), true)) {
        ...
        if (! empty($wmpl_id)) {
            ...
            $single_string_types = apply_filters('ot_wpml_option_types', array( 'text', 'textarea', 'textarea-simple', 'wpeditor'  ));
            ...
            if ($find_table === $table_name) {
                ...
                foreach ($old_settings as $setting) {
                    ...
                    if ('heading' === $setting->item_type) {
                        ...
                        // Textarea rows.
                        $rows = '';
                        if (in_array($settings['settings'][ $settings_count ]['type'], array( 'css', 'javascript', 'textarea' , 'wpeditor' ), true)) {
...
if (! function_exists('ot_settings_view')) {
    ...
    function ot_settings_view($name, $key, $setting = array())
    {
        ...
        if (in_array($type, array( 'css', 'javascript', 'textarea', 'textarea-simple' , 'wpeditor' ), true)) {
    ...
    function ot_list_item_view($name, $key, $list_item = array(), $post_id = 0, $get_option = '', $settings = array(), $type = '')
    {
        ...
        foreach ($settings as $field) {
            ...
            $_args = array(
				...
				'is_number'          => isset($field['is_number']) ? $field['is_number'] : '',
            );
